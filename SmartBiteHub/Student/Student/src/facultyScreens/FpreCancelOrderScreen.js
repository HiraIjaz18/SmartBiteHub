...
const FpreCancelOrderScreen = ({ route, navigation }) => {
  ...
  const initSocket = async () => {
    try {
      ...
      socketService.subscribe('pre_order_cancelled', (data) => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (socketRef.current) {
        ...
        socketRef.current.unsubscribe('pre_order_cancelled');
      }
    };
  }, []);
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        const orderValid = await validateSession();
        console.log('[Initialization] Order valid:', orderValid);
  ...
  const handleSuccessfulCancellation = async (data) => {
    ...
    Alert.alert(
      'Order Cancelled', 
      `Your order has been successfully cancelled. Refunded: ₹${data.refundAmount}`,
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      await socketRef.current.joinOrderRoom(orderId, email);
      
      console.log('[API] Sending cancellation request');
      const response = await fetch('http://172.21.12.246:4000/api/Fprecancel/Fcancel', {
        method: 'POST',
      ...
      <LinearGradient colors={['#FF6F61', '#FFD166']} style={styles.container}>
        <View style={styles.content}>
          ...
          <Text style={styles.errorText}>
            This order cannot be cancelled or has already been processed.
          </Text>
          <TouchableOpacity
            style={styles.button}
  ...
  return (
    <LinearGradient colors={['#FF6F61', '#FFD166']} style={styles.container}>
      <View style={styles.content}>
        <Text style={styles.title}>Cancel Pre-Order</Text>
...
export default FpreCancelOrderScreen;