...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('http://172.21.12.17:4000/api/student/login', {
        method: 'POST',
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 32, fontWeight: 'bold', color: '#B24D32', marginBottom: 10 },
  subtitle: { fontSize: 16, color: '#6E7D4B', marginBottom: 30 },
  input: {
    width: '100%', padding: 15, marginBottom: 15,
    borderColor: '#E0E0E0', borderWidth: 1, borderRadius: 10,
    backgroundColor: '#FFF', fontSize: 16, color: '#333',
  ...
  button: { backgroundColor: '#F7B267', padding: 15, borderRadius: 10, width: '100%', alignItems: 'center' },
  buttonText: { color: '#FFF', fontSize: 16, fontWeight: 'bold' },
  forgotPasswordText: { marginTop: 10, color: '#6E7D4B' },
  linkText: { marginTop: 15, color: '#6E7D4B' },
  linkHighlight: { color: '#B24D32', fontWeight: 'bold' }
});
